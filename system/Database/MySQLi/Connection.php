...
{
    public function _transBegin(): bool
    {
    ...
    public function _transCommit(): bool
    {
        // Code to commit a transaction
        return true;
    }
    
    public function _transRollback(): bool
    {
        // Code to rollback a transaction
        return true;
    }
...
{
    // Code to escape a value for use in a query
}

public function _quote($value)
{
    // Code to quote a value for use in a query
}

public function _getSQL($sql)
{
    // Code to get the SQL for a query
}

public function _insertID()
{
    // Code to get the ID of the last inserted row
}

public function _error()
{
    // Code to get the error message from the database
}
...
{
    // Code to check if the connection to the database is alive
}
public function affectedRows(): int
{
    // Code to retrieve the number of affected rows
    return $this->connID->affected_rows;
}

public function error(): array
{
    // Code to retrieve the last error message
...
{
    // Code to retrieve the last inserted ID
   ...
   // }
    ...
    protected function execute(string $sql)
    {
        while ($this->connID->more_results()) {
            ...
            // Closing brace for the preceding code block
            
            // Assuming this is inside a function or method
            try {
                // Set the resultMode parameter to MYSQLI_NO_RESULT,
                // which will prevent the query from returning a result set.
                define('MYSQLI_NO_RESULT', 0);
            
                if ($res = $this->connID->store_result()) {
                    $res->free();
                }
            
                // Place the code that throws exceptions inside the try block
                ...
                if ($this->DBDebug) {
                    ...
                    // Handle the specific error when the 'name' column is null
                    if ($e->getCode() === 1048 && strpos($e->getMessage(), "'name'") !== false) {
            ...
            } // Move the closing brace for the try block here
            
            // Handle any additional exception types if needed
            catch (Exception $e) {
            ...
            return false;
                 ...
                 */
                function prepQuery(string $sql): string
                {
                 ...
                 */
                 function _escapeString(string $str): string
                {
                    if (! $this->connID) {
                        $this->initialize();
                    }
                 ...
                 */
                function escapeLikeStringDirect($str)
                {
                    if (is_array($str)) {
     ...
     */
     function _listTables(bool $prefixLimit = false, ?string $tableName = null): string
    {
     ...
     */
     function _listColumns(string $table = ''): string
    {
     ...
     * @throws DatabaseException
     */
    function _fieldData(string $table): array
    {
     ...
     */
    function _indexData(string $table): array
    {
        $table = $this->protectIdentifiers($table, true, null, false);
     ...
     * @throws DatabaseException
     */
  function _foreignKeyData(string $table): array
    {
        $sql = '
                SELECT
                    tc.CONSTRAINT_NAME,
     ...
     * @return string
     */
    function _disableForeignKeyChecks()
    {
     ...
     * @return string
     */
    function _enableForeignKeyChecks()
    {
    ...
    function _transRollback(): bool
     {
         ...
         } // Closing brace for the function or method
        
        // Additional code here if needed
        
        } // Closing brace for the class or code block
    }
}